...
from audio_process import load_wav, spectrogram, melspectrogram
...
class LJSpeechDataset(data.Dataset):
    ...
    def __init__(self, wav_path, csv_file,
                 text_transformer=text_to_sequence, audio_transformer=spectrogram,
    ...
    def _get_text(self, index):
        text = self.metadata.iloc[index]['normalized_text']
    ...
    def _get_audio(self, index):
        filepath = self.metadata.iloc[index]['file_path']
        audio = load_wav(filepath, self.sample_rate)
...
if __name__ == '__main__':
    ...
    dataset = LJSpeechDataset(wav_path, "data/train.csv", text_transformer=text_to_sequence,
