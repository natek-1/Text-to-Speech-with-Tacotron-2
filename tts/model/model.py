...
class Encoder(nn.Module):
    ...
    def forward(self, text_padded, input_lengths):
        ...
        packed_input = pack_padded_sequence(x, input_lengths.cpu(), batch_first=True)
...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_padded_outputs, encoder_mask,
        ...
        feat_outputs = feat_outputs.masked_fill(decoder_mask.bool(), 0.0)
        feat_residual_outputs = feat_residual_outputs.masked_fill(decoder_mask.bool(), 0.0)
        attention_weights = attention_weights.masked_fill(decoder_mask.bool(), 0.0)
        stop_tokens = stop_tokens.masked_fill(decoder_mask.squeeze().bool(), 1e3)  # sigmoid(1e3) = 1, log(1) = 0
        return feat_outputs, feat_residual_outputs, stop_tokens, attention_weights
...
class LocationSensitiveAttention(nn.Module):
    ...
    def _cal_energy(self, query, values, cumulative_attention_weights, mask=None):
        ...
        if mask is not None:
            energies = energies.masked_fill(mask.bool(), -np.inf)
        # print(energies)
        return energies
